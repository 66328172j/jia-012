...
public class PestSurveyController extends BaseController {
    ...
    public String edit(@PathVariable("id") Long id, ModelMap model) {
        ...
        // 已复核定稿记录只读展示：可回看当时定值，但不允许提交修改
        model.put("locked", survey != null && survey.getSurveyStatus() != null && survey.getSurveyStatus() == 1);
    ...
    public AjaxResult editSave(TPestSurvey record) {
        ...
        // 已复核记录已定稿，禁止再走编辑改数；只能由复核动作推进状态
        if (dbRecord.getSurveyStatus() != null && dbRecord.getSurveyStatus() == 1) {
            return error("该调查记录已复核定稿，不能修改");
        }
